...
```
3. Add the following function to **UserSearchController.cs**. This function is passed a userId from the user selected in the table, and builds the query to get the full profile information of the user and their photo. This is passed to a new view through a Profile object.
...
public async Task<ActionResult> ShowProfile(string userId)
{
    ...
    try {
        ...
        if (photo != null)
        {
            ...
            {
                string toBase64 = ToBase64(stream);
                profile.photo = "data:image/jpeg;base64, " + toBase64;
            }
        }
    }
    catch (Exception)
...
```
You'll also need to include the following too:
```csharp
